...
s = Service(r"/Users/jingszhou/Iclicker/Auto-iClicker/src/chromedriver")
...
while not button_found and retries < max_retries:
    try:
        ...
        print(f"Join button not found, retrying... ({retries + 1}/{max_retries})")
        retries += 1
        time.sleep(retry_interval)

if not button_found:
    print("Failed to find the Join button after several attempts. Exiting.")
    driver.quit()
    exit()
    ...
    if driver.current_url == 'https://student.iclicker.com/#/polling':
        ...
        while not clicked and retries < max_retries:
            try:
                ...
                print("Click failed, retrying... Attempt:", retries + 1)
                retries += 1
                time.sleep(1)  # Wait before retrying
                # Re-check the button element after waiting
        ...
        if not clicked:
            print("Failed to click Multiple Choice A after several attempts.")
